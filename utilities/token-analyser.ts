...
>() {
  ...
  class TokenAnalyser
    ...
    private startNewTransition() {
      ...
      this.currentTransition = ((a) => a[a.length - 1])(
    ...
    toAnyOf(group: TGroup, ...args: TGroup[]) {
      if (
        !this.currentKey || !this.currentTransition ||
    ...
    analyse(value: string, startingToken: TToken): ParsedToken<TToken>[] {
      ...
      for (const nextCharacter of value.trim().slice(1) + "\0") {
        const transition: [TGroup, TGroup] = [
          currentGroup,
        ...
        const result = candidates?.filter((c) =>
          c.from.includes(transition[0]) && c.to.includes(transition[1])
        )[0];
        if (!result) {
          throw Error(
            "Illegal transition " + JSON.stringify(transition) +
              " when parsing " + currentToken,
          );
        }
